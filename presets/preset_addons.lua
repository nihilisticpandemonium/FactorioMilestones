        ...
        build_func = function(self)
          for _, science in pairs(game.item_prototypes) do
            if science and science.type == "tool" and science.name:sub(1, 8) == "infused-" then
              ...
              table.insert(self.milestones, {type = "item", name = science.name, quantity = 1})
              table.insert(self.milestones, {type = "item", name = science.name, quantity = 1000, next = "x10"})
            end
          end
        end
    },
